...
void LcdDisplay::SetCameraPreviewImage(std::unique_ptr<LvglImage> image) {
    DisplayLockGuard lock(this);
    if (preview_image_ == nullptr || image == nullptr) return;
    ...
    lv_obj_align(preview_image_, LV_ALIGN_CENTER, 0, 0);
    lv_image_set_scale(preview_image_, 256);  // 1:1, no scaling
...
void LcdDisplay::StopCameraPreview() {
    ...
    esp_timer_stop(preview_timer_);
    lv_obj_add_flag(preview_image_, LV_OBJ_FLAG_HIDDEN);
    // Restore preview_image_ to its original half-size
    lv_obj_set_size(preview_image_, width_ / 2, height_ / 2);
    preview_image_cached_.reset();
    lv_obj_remove_flag(emoji_box_, LV_OBJ_FLAG_HIDDEN);
