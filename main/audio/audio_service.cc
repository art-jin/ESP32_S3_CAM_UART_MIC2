...
bool AudioService::ReadAudioData(std::vector<int16_t>& data, int sample_rate, int samples) {
    ...
    if (codec_->input_sample_rate() != sample_rate) {
        int raw_ch = codec_->raw_input_channels();
        data.resize(samples * codec_->input_sample_rate() / sample_rate * raw_ch);
        if (!codec_->InputData(data)) {
            return false;
        }

        if (raw_ch == 4) {
            // 4-channel TDM from ES7210: [MIC1, MIC2, MIC3, MIC4, MIC1, ...]
            size_t frames = data.size() / 4;
            auto ch0 = std::vector<int16_t>(frames);
            auto ch1 = std::vector<int16_t>(frames);
            auto ch2 = std::vector<int16_t>(frames);
            auto ch3 = std::vector<int16_t>(frames);
            for (size_t i = 0, j = 0; i < frames; ++i, j += 4) {
                ch0[i] = data[j];
                ch1[i] = data[j + 1];
                ch2[i] = data[j + 2];
                ch3[i] = data[j + 3];
            ...
            // Diagnostic: log all 4 channel energies
            // SLOT0=physical mic1, SLOT1=AEC reference, SLOT2=physical mic2, SLOT3=unused
            static int diag_count = 0;
            if (++diag_count <= 20 || diag_count % 500 == 0) {
                float e0 = 0, e1 = 0, e2 = 0, e3 = 0;
                for (size_t i = 0; i < frames; i++) {
                    e0 += (float)ch0[i] * ch0[i];
                    e1 += (float)ch1[i] * ch1[i];
                    e2 += (float)ch2[i] * ch2[i];
                    e3 += (float)ch3[i] * ch3[i];
                }
                e0 /= frames; e1 /= frames; e2 /= frames; e3 /= frames;
                ESP_LOGI("TDM_Diag", "4ch[%d]: MIC1(S0)=%.0f AEC_REF(S1)=%.0f MIC2(S2)=%.0f NC(S3)=%.0f",
                         diag_count, e0, e1, e2, e3);
            }

            // Feed DOA with 2 physical mics: SLOT0 (mic1) + SLOT2 (mic2)
            // Skip when AEC reference (SLOT1) has high energy = speaker is playing
            auto* doa = DoaTracker::GetInstance();
            if (doa) {
                float aec_energy = 0;
                for (size_t i = 0; i < frames; i++) {
                    aec_energy += (float)ch1[i] * ch1[i];
                }
                aec_energy /= frames;
                if (aec_energy < 5000) {  // speaker not active, feed DOA
                    auto doa_data = std::vector<int16_t>(frames * 2);
                    for (size_t i = 0, j = 0; i < frames; ++i, j += 2) {
                        doa_data[j] = ch0[i];     // physical mic 1
                        doa_data[j + 1] = ch2[i]; // physical mic 2
                    }
                    doa->FeedAudio(doa_data.data(), frames);
                }
            }

            // Resample mic (SLOT0) and AEC reference (SLOT1) for AFE
            auto mic_channel = ch0;
            auto reference_channel = ch1;
        ...
        } else if (codec_->input_channels() == 2) {
        ...
        data.resize(samples * codec_->raw_input_channels());
        if (!codec_->InputData(data)) {
            return false;
        }
        if (codec_->raw_input_channels() == 4 && codec_->input_channels() == 2) {
            // De-interleave 4 raw channels
            // SLOT0=physical mic1, SLOT1=AEC reference, SLOT2=physical mic2, SLOT3=unused
            size_t frames = data.size() / 4;
            auto mic_channel = std::vector<int16_t>(frames);
            auto reference_channel = std::vector<int16_t>(frames);
            for (size_t i = 0, j = 0; i < frames; ++i, j += 4) {
                mic_channel[i] = data[j];           // SLOT0: physical mic 1
                reference_channel[i] = data[j + 1]; // SLOT1: AEC reference
            }

            // Feed DOA with SLOT0 (mic1) + SLOT2 (mic2) — the 2 physical mics
            // Skip when AEC reference (SLOT1) has high energy = speaker is playing
            auto* doa = DoaTracker::GetInstance();
            if (doa) {
                float aec_energy = 0;
                for (size_t i = 0; i < frames; i++) {
                    aec_energy += (float)reference_channel[i] * reference_channel[i];
                }
                aec_energy /= frames;
                if (aec_energy < 5000) {  // speaker not active, feed DOA
                    auto doa_data = std::vector<int16_t>(frames * 2);
                    for (size_t i = 0, j = 0; i < frames; ++i, j += 2) {
                        doa_data[j] = data[i * 4];       // SLOT0: physical mic 1
                        doa_data[j + 1] = data[i * 4 + 2]; // SLOT2: physical mic 2
                    }
                    doa->FeedAudio(doa_data.data(), frames);
                }
            }

            // Re-interleave for AFE: mic (SLOT0) + reference (SLOT1)
