...
Esp32Camera::~Esp32Camera() {
    if (camera_mutex_) {
        vSemaphoreDelete(camera_mutex_);
        camera_mutex_ = nullptr;
    }
...
bool Esp32Camera::Capture() {
    ...
    if (camera_mutex_ && xSemaphoreTake(camera_mutex_, pdMS_TO_TICKS(500)) != pdTRUE) {
        ESP_LOGW(TAG, "Capture: camera mutex timeout");
        return false;
    }
...
void Esp32Camera::StartPreview(Display* display) {
    if (preview_running_ || video_fd_ < 0 || !streaming_on_) return;

    uint16_t w = frame_.width ? frame_.width : 320;
    uint16_t h = frame_.height ? frame_.height : 240;
    // Downsample to half resolution for smoother display
    uint16_t out_w = w / 2;
    uint16_t out_h = h / 2;
    size_t out_size = out_w * out_h * 2;

    // Full-res scratch buffer for color conversion, half-res output buffer
    size_t full_size = w * h * 2;
    uint8_t* full_buf = (uint8_t*)heap_caps_malloc(full_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    ...
    if (!full_buf || !out_buf) {
        ...
        if (full_buf) heap_caps_free(full_buf);
    ...
    PreviewCtx* ctx = new PreviewCtx{this, display, full_buf, out_buf, full_size, out_size, w, h, out_w, out_h};

    auto task_func = [](void* arg) {
        PreviewCtx* ctx = static_cast<PreviewCtx*>(arg);
        Esp32Camera* self = ctx->cam;
        Display* disp = ctx->disp;
        uint8_t* full_buf = ctx->full_buf;
        uint8_t* out_buf = ctx->out_buf;
        uint16_t src_w = ctx->src_w;
        uint16_t src_h = ctx->src_h;
        uint16_t out_w = ctx->out_w;
        uint16_t out_h = ctx->out_h;
        size_t out_size = ctx->out_size;
        size_t full_size = ctx->full_size;
        delete ctx;

        ESP_LOGI(TAG, "Camera preview started, fmt=0x%08lx, %dx%d -> %dx%d",
                 (unsigned long)self->sensor_format_, src_w, src_h, out_w, out_h);

        while (self->preview_running_) {
            if (self->camera_mutex_ && xSemaphoreTake(self->camera_mutex_, pdMS_TO_TICKS(500)) != pdTRUE) {
            ...
            if (ioctl(self->video_fd_, VIDIOC_DQBUF, &buf) != 0) {
                if (self->camera_mutex_) xSemaphoreGive(self->camera_mutex_);
                vTaskDelay(pdMS_TO_TICKS(10));
                continue;
            }

            // Get full-res RGB565 data into full_buf
            if (self->sensor_format_ == V4L2_PIX_FMT_RGB565) {
                memcpy(full_buf, self->mmap_buffers_[buf.index].start,
                       std::min(full_size, self->mmap_buffers_[buf.index].length));
            } else {
                esp_imgfx_color_convert_cfg_t convert_cfg = {};
                convert_cfg.in_res.width = src_w;
                convert_cfg.in_res.height = src_h;
                convert_cfg.in_pixel_fmt = static_cast<esp_imgfx_pixel_fmt_t>(self->sensor_format_);
                convert_cfg.out_pixel_fmt = ESP_IMGFX_PIXEL_FMT_RGB565_LE;
                convert_cfg.color_space_std = ESP_IMGFX_COLOR_SPACE_STD_BT601;
                esp_imgfx_color_convert_handle_t handle = nullptr;
                if (esp_imgfx_color_convert_open(&convert_cfg, &handle) == ESP_IMGFX_ERR_OK && handle) {
                    esp_imgfx_data_t in_data = {};
                    in_data.data = (uint8_t*)self->mmap_buffers_[buf.index].start;
                    in_data.data_len = self->mmap_buffers_[buf.index].length;
                    esp_imgfx_data_t out_data = {};
                    out_data.data = full_buf;
                    out_data.data_len = static_cast<uint32_t>(full_size);
                    esp_imgfx_color_convert_process(handle, &in_data, &out_data);
            ...
            if (self->camera_mutex_) {
                xSemaphoreGive(self->camera_mutex_);
            }

            // Downsample 2x2 -> 1x1 by taking every other pixel in both dimensions
            uint16_t* src = (uint16_t*)full_buf;
            uint16_t* dst = (uint16_t*)out_buf;
            for (int y = 0; y < out_h; y++) {
                uint16_t* src_row = src + (y * 2) * src_w;
                for (int x = 0; x < out_w; x++) {
                    *dst++ = src_row[x * 2];
                }
            }

            // Allocate LVGL image from the downsampled buffer
            uint8_t* img_data = (uint8_t*)heap_caps_malloc(out_size, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
            if (img_data) {
                memcpy(img_data, out_buf, out_size);
                size_t stride = ((out_w * 2) + 3) & ~3;
                auto image = std::make_unique<LvglAllocatedImage>(img_data, out_size, out_w, out_h, stride, LV_COLOR_FORMAT_RGB565);
                auto lvgl_disp = dynamic_cast<LvglDisplay*>(disp);
                if (lvgl_disp) {
                    lvgl_disp->SetCameraPreviewImage(std::move(image));
                }
            }
        ...
        auto lvgl_disp = dynamic_cast<LvglDisplay*>(disp);
        if (lvgl_disp) {
            lvgl_disp->StopCameraPreview();
        }
        ESP_LOGI(TAG, "Camera preview stopped");
        vTaskDelete(NULL);
    };

    xTaskCreatePinnedToCore(task_func, "cam_preview", 4096, ctx, 1, &preview_task_, 1);
}
...
bool Esp32Camera::CaptureRawRGB565(uint8_t* dst, size_t dst_len, uint16_t* out_w, uint16_t* out_h) {
    if (!streaming_on_ || video_fd_ < 0 || !dst || !dst_len) {
        return false;
    }
    ...
    if (ioctl(video_fd_, VIDIOC_DQBUF, &buf) != 0) {
        if (camera_mutex_) xSemaphoreGive(camera_mutex_);
        return false;
    }

    uint16_t w = frame_.width;
    uint16_t h = frame_.height;
    size_t required = (size_t)w * h * 2;

    if (sensor_format_ == V4L2_PIX_FMT_RGB565) {
        size_t copy_len = std::min({dst_len, required, mmap_buffers_[buf.index].length});
        memcpy(dst, mmap_buffers_[buf.index].start, copy_len);
    } else {
        esp_imgfx_color_convert_cfg_t convert_cfg = {};
        convert_cfg.in_res.width = w;
        convert_cfg.in_res.height = h;
        convert_cfg.in_pixel_fmt = static_cast<esp_imgfx_pixel_fmt_t>(sensor_format_);
        convert_cfg.out_pixel_fmt = ESP_IMGFX_PIXEL_FMT_RGB565_LE;
        convert_cfg.color_space_std = ESP_IMGFX_COLOR_SPACE_STD_BT601;
        esp_imgfx_color_convert_handle_t handle = nullptr;
        if (esp_imgfx_color_convert_open(&convert_cfg, &handle) == ESP_IMGFX_ERR_OK && handle) {
            esp_imgfx_data_t in_data = {};
            in_data.data = (uint8_t*)mmap_buffers_[buf.index].start;
            in_data.data_len = mmap_buffers_[buf.index].length;
            esp_imgfx_data_t out_data = {};
            out_data.data = dst;
            out_data.data_len = static_cast<uint32_t>(std::min(dst_len, required));
            esp_imgfx_color_convert_process(handle, &in_data, &out_data);
            esp_imgfx_color_convert_close(handle);
        }
    }
